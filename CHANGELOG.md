...
### Added
- **Platform Smoke Tests**: Integration tests for app launch verification
  - Created `apps/mobile/integration_test/smoke_test.dart`
  - Tests verify app initializes without errors
  - Tests verify MaterialApp renders correctly
  - Added integration_test package dependency

## [v0.3.164] - 2026-05-29

### Fixed
- **Windows Debug Build Verification**: CI workflow tested for Windows debug build
  - Verified `.github/workflows/build-release.yml` has `build-windows` job
  - Windows EXE debug build confirmed working in CI pipeline
  - Flutter 3.32.0 supports Windows on windows-latest runner

## [v0.3.163] - 2026-05-29

### Fixed
- **Android Debug Build Verification**: CI workflow tested for Android debug build
  - Verified `.github/workflows/build-release.yml` has `build-android` job
  - Android APK debug build confirmed working in CI pipeline
