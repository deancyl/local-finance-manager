...
/// Provider for income statement data with date range filtering
final incomeStatementProvider = AsyncNotifierProvider<IncomeStatementNotifier, IncomeStatement?>(
  () => IncomeStatementNotifier(),
);

/// Notifier for managing income statement state
class IncomeStatementNotifier extends AsyncNotifier<IncomeStatement?> {
  late final LocalFinanceDatabase _db;
  late final IncomeStatementCalculator _calculator;

  @override
  IncomeStatement? build() {
  ...
  Future<IncomeStatement> _fetch() async {
    ...
    final incomeStatement = await _calculator.calculate(
      accounts: accounts,
      balances: balances,
    ...
    // Update state
    state = AsyncValue.data(incomeStatement);
