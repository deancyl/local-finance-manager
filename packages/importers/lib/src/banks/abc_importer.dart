...
///
/// Supports ABC bank statement exports with:
/// - GBK or UTF-8 encoding
/// - Headers: 交易日期, 交易金额, 账户余额, 交易摘要, etc.
/// - Date format: 20260519 or 2026-05-19
/// - Amount can be positive/negative
class AbcImporter extends ImporterBase {
  @override
  String get name => '农业银行';

  @override
  String get sourceId => 'abc';
  ...
  @override
  String get description => '农业银行账户交易记录导入';

  /// ABC-specific header patterns for detection.
  ...
  bool canParse({
    ...
    for (var i = 0; i < lines.length && i < 5; i++) {
      ...
      // Also check for ABC-specific patterns
      if (line.contains('农业银行') || line.contains('ABC') || line.contains('中国农业银行')) {
        return true;
      }
    }
  ...
  Map<String, int> _buildHeaderMap(List<String> headers) {
    ...
    for (var i = 0; i < headers.length; i++) {
      ...
      // Income column
      if (header == '收入' || header == '存入金额' || header == '存入') {
        map['income'] ??= i;
      }

      // Expense column
      if (header == '支出' || header == '取出金额' || header == '取出') {
        map['expense'] ??= i;
      }
  ...
  ParsedTransaction? _parseRow(
    ...
    final externalId = 'abc_${date.millisecondsSinceEpoch}_${amount.abs()}_$rowNum';
