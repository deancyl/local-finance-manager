...
///
/// Supports BOCOM bank statement exports with:
/// - GBK or UTF-8 encoding
/// - Headers: 交易日期, 交易金额, 账户余额, 交易摘要, etc.
/// - Date format: 20260519 or 2026-05-19
/// - Amount can be positive/negative
class BocomImporter extends ImporterBase {
  @override
  String get name => '交通银行';

  @override
  String get sourceId => 'bocom';
  ...
  @override
  String get description => '交通银行账户交易记录导入';

  /// BOCOM-specific header patterns for detection.
  ...
  bool canParse({
    ...
    for (var i = 0; i < lines.length && i < 5; i++) {
      ...
      // Also check for BOCOM-specific patterns
      if (line.contains('交通银行') || line.contains('BOCOM') || line.contains('交行')) {
        return true;
      }
    }
  ...
  ParsedTransaction? _parseRow(
    ...
    final externalId = 'bocom_${date.millisecondsSinceEpoch}_${amount.abs()}_$rowNum';
